...
const initSource = await readFile(new URL('../src/behaviors/theme-init.js', import.meta.url), 'utf8');
const behaviorSource = await readFile(new URL('../src/behaviors/theme.js', import.meta.url), 'utf8');

function environment({ dark = false, saved = null, blocked = false, reduced = false, transitions = false } = {}) {
  const values = new Map(saved ? [['antony-theme', saved]] : []);
  const events = {};
  const root = { dataset: {}, animate: () => ({ finished: Promise.resolve() }) };
  const button = { hidden: true, label: {}, querySelector() { return this.label; }, setAttribute(name, value) { this[name] = value; }, addEventListener(name, fn) { events[name] = fn; }, getBoundingClientRect() { return { x: 100, y: 20, width: 160, height: 48 }; } };
  const system = { matches: dark, addEventListener(name, fn) { events.system = fn; } };
  let transitionCount = 0;
  const document = {
    documentElement: root,
    querySelector: (selector) => selector === '#theme-switch' ? button : { content: '' },
    startViewTransition: transitions ? (fn) => { transitionCount++; fn(); return { ready: Promise.resolve(), finished: Promise.resolve() }; } : undefined,
  };
  const localStorage = {
  ...
  const context = { document, localStorage, matchMedia: (query) => query.includes('reduced') ? { matches: reduced } : system, window: { addEventListener(name, fn) { events[name] = fn; } }, innerWidth: 1280, innerHeight: 800 };
  runInNewContext(initSource, context);
  const initialTheme = root.dataset.theme;
  runInNewContext(behaviorSource.replace('export function', 'function') + '\ninitTheme();', context);
  return { root, events, values, system, button, initialTheme, transitions: () => transitionCount };
}
...
test('saved manual choice wins before rendering and across new sessions', async () => {
  ...
  assert.equal(environment({ dark: true, saved: app.values.get('antony-theme') }).initialTheme, 'garden');
});
test('invalid or inaccessible storage does not break theme controls', async () => {
  for (const options of [{ saved: 'unknown', dark: true }, { blocked: true, dark: true }]) {
...
test('reduced motion skips View Transitions; unsupported browsers switch directly', async () => {
  for (const options of [{ reduced: true, transitions: true }, { transitions: false }]) {
    const app = environment(options);
    await app.events.click();
