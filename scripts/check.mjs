...
const themePosition = html.search(
  /localStorage\.getItem\(["']antony-theme["']\)/,
);
assert.ok(
  themePosition >= 0 && themePosition < html.indexOf('rel="stylesheet"'),
  "Theme must be resolved before CSS",
);
...
for (const icon of JSON.parse(await readFile("dist/site.webmanifest", "utf8"))
  .icons)
  await access(join("dist", icon.src));
let jsBytes = 0;
let cssBytes = 0;
...
console.log(
  `Structure, anchors, assets, metadata: OK. Gzip: JS ${jsBytes} B · CSS ${cssBytes} B.`,
);
