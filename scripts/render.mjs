...
await writeFile(new URL('../index.html', import.meta.url), `<!doctype html><html lang="fr"><head>${head}</head><body>${renderNavigation()}<main id="contenu" tabindex="-1">${renderHero()}${renderValue()}${renderAchievements()}${renderExpertise()}${renderCareer()}${renderMethod()}${renderAbout()}${renderContact()}</main>${renderFooter()}<script type="module" src="/src/main.js"></script></body></html>`);
